...
extension VideoPlayer {
    ...
    func switchTrack(index: Int, isFirstTime: Bool = false) {
        ...
        // Fetch available tracks safely
        let videoTracks = asset.tracks(withMediaType: .video)
        let subtitleTracks = asset.tracks(withMediaType: .subtitle)
        
        // Ensure we have at least one video track
        guard !videoTracks.isEmpty else {
            print("VideoPlayer: No video tracks found in asset")
            return
        }
        
        // Determine safe indices
        let safeVideoIndex = min(max(index, 0), videoTracks.count - 1)
        let videoTrack = videoTracks[safeVideoIndex]
        
        // Create composition video track and insert
        guard let compositionVideoTrack = composition.addMutableTrack(withMediaType: .video, preferredTrackID: kCMPersistentTrackID_Invalid) else {
            print("VideoPlayer: Failed to create composition video track")
            return
        }
        ...
        do {
            ...
            print("VideoPlayer: Failed to insert video track into composition - \(error.localizedDescription)")
            return
        }
        
        // If we have subtitle tracks, try to add the selected one; otherwise skip
        if !subtitleTracks.isEmpty {
            let safeSubtitleIndex = min(max(index, 0), subtitleTracks.count - 1)
            let subTrack = subtitleTracks[safeSubtitleIndex]
            
            if let subtitleTrack = composition.addMutableTrack(withMediaType: .text, preferredTrackID: kCMPersistentTrackID_Invalid) {
                do {
                    try subtitleTrack.insertTimeRange(CMTimeRange(start: .zero, duration: asset.duration), of: subTrack, at: .zero)
                } catch {
                    print("VideoPlayer: Failed to insert subtitle track - \(error.localizedDescription)")
                }
            } else {
                print("VideoPlayer: Failed to create composition subtitle track")
            }
        } else {
            // No subtitle track available; this is acceptable
            print("VideoPlayer: No subtitle tracks found; continuing without subtitles")
        }
